...
class Client:
    ...
    def split_msg(self, msg, mtype, mformat):
    
        chunks = [msg[i: i+util.CHUNK_SIZE] for i in range(0, len(msg), util.CHUNK_SIZE)]
        pkt = []

        for chunk in chunks:
            segment = util.make_message(mtype, mformat, chunk)
            packet = util.make_packet("data", self.seq, segment)
            pkt.append(packet)
            self.seq += 1
        
        return pkt

    def check_ack(self, seqnum, addr, ptype, data=None):

        #use retrans timer to wait for an ack
        ack_received = False
        start_time = time.time()

        while not ack_received and (time.time() - start_time) < util.TIME_OUT:
            try:
                self.sock.settimeout(util.TIME_OUT - (time.time() - start_time))
                ack_data, _ = self.sock.recvfrom(2048)
                ack_msg_type, ack_seq, _, _ = util.parse_packet(ack_data.decode())
                if ack_msg_type == "ack" and int(ack_seq) == seqnum:
                    ack_received = True
                    return True
                ...
                if ptype == "start":
                    start_packet = util.make_packet("start", seqnum, "")
                    self.sock.sendto(start_packet.encode(), addr)
                elif ptype == "data":
                    data_packet = util.make_packet("data", seqnum, data)
                    self.sock.sendto(data_packet.encode(), addr)
                elif ptype == "end":
                    end_packet = util.make_packet("end", seqnum, "")
